// Walking pedestrian (crosswalk-style) icon. Lucide doesn't ship one
// so we render the silhouette directly via react-native-svg.
//
// API matches lucide icons (`size`, `color`) so it's a drop-in
// replacement at the call site. `flip` mirrors the figure horizontally
// for callers that want it facing the other direction.
...
interface Props {
  size?: number;
  color?: string;
  /** Mirror the figure horizontally (face the other direction). */
  flip?: boolean;
}
...
export function WalkIcon({
  ...
  const transform = flip ? `scale(-1, 1) translate(-${VIEWBOX}, 0)` : undefined;
  return (
