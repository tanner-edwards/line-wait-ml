"""Compute per-ride closure duration + delta profiles and write to Firestore.

Reads closure_events.csv (output of closure_analysis.py) and writes one doc
per ride to closure_profiles/{ride_id}.

Run from repo root:
  GOOGLE_APPLICATION_CREDENTIALS=firebase-key.json \\
    /Users/tannere/Projects/line-wait-ml/.venv/bin/python scripts/build_closure_profiles.py
"""
import os
import sys
from datetime import datetime, timezone

import firebase_admin
from firebase_admin import credentials, firestore
import pandas as pd
import numpy as np

cred_path = os.environ.get("GOOGLE_APPLICATION_CREDENTIALS")
if not cred_path:
    print("Set GOOGLE_APPLICATION_CREDENTIALS=firebase-key.json")
    sys.exit(1)

csv_path = os.path.join(os.path.dirname(__file__), "closure_events.csv")
if not os.path.exists(csv_path):
    print(f"closure_events.csv not found. Run closure_analysis.py first.")
    sys.exit(1)

df = pd.read_csv(csv_path)
print(f"Read {len(df)} closure events")

firebase_admin.initialize_app(credentials.Certificate(cred_path))
db = firestore.client()

now_iso = datetime.now(timezone.utc).isoformat()
written = 0
skipped = 0

for ride_id, grp in df.groupby("ride_id"):
    ride_name = grp["ride_name"].iloc[0]
    durations = grp["duration_min"].dropna().values
    n = len(durations)

    if n < 5:
        skipped += 1
        continue

    # Short reset threshold: 70th percentile of closure durations, clamped.
    # Rides above this line have "escaped" the brief-reset cluster.
    threshold = float(np.clip(np.percentile(durations, 70), 15.0, 120.0))

    # Extended closures: those that crossed the threshold
    ext = grp[grp["duration_min"] > threshold]
    ext_durations = ext["duration_min"].dropna().values
    ext_n = len(ext_durations)

    ext_p50 = round(float(np.percentile(ext_durations, 50)), 1) if ext_n >= 3 else None
    ext_p75 = round(float(np.percentile(ext_durations, 75)), 1) if ext_n >= 3 else None

    ext_deltas = ext["delta"].dropna().values
    ext_median_delta = round(float(np.median(ext_deltas)), 1) if len(ext_deltas) >= 3 else None

    doc = {
        "rideId": ride_id,
        "rideName": ride_name,
        "sampleCount": int(n),
        "shortResetThresholdMin": round(threshold, 1),
        "p50Min": round(float(np.percentile(durations, 50)), 1),
        "p75Min": round(float(np.percentile(durations, 75)), 1),
        "p90Min": round(float(np.percentile(durations, 90)), 1),
        "extendedSampleCount": int(ext_n),
        "extendedP50Min": ext_p50,
        "extendedP75Min": ext_p75,
        "extendedMedianDelta": ext_median_delta,
        "updatedAt": now_iso,
    }

    db.collection("closure_profiles").document(ride_id).set(doc)
    written += 1
    print(f"  {ride_name[:45]:<45}  threshold={threshold:5.0f}m  n={n:4d}  ext_n={ext_n:3d}  ext_p50={str(ext_p50)+'m':>7}")

print(f"\nWrote {written} profiles, skipped {skipped} rides (< 5 events)")
